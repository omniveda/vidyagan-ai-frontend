...
const StatsComponent = () => {
  ...
  return (
    <div className="bg-black-700" ref={setElement}>
      {/* Stats */}
          ...
          {stats.map((data, index) => (
            <div className="flex flex-col py-8" key={index}>
              <div className="flex flex-row px-20">
                <h1
                  className="text-[30px] font-bold text-yellow-5"
                  ref={(el) => (elementsRef.current[index] = el)}
                >
                  {parseCountValue(data.count).toLocaleString()}
                </h1>
                <span className="text-[30px] font-bold text-yellow-5"> + </span>
              </div>
              <div className="flex flex-row px-20">
                <h2 className="font-semibold text-[16px] text-black-500">
                  {data.label}
                </h2>
              </div>
            </div>
